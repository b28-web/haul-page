defmodule Haul.AI.EditApplierTest do
  ...
  setup do
    ...
    on_exit(fn ->
      Ecto.Adapters.SQL.query(Haul.Repo, ~s(DROP SCHEMA IF EXISTS "#{result.tenant}" CASCADE))
    end)
    ...
    %{tenant: result.tenant, profile: profile}
  end

  describe "apply_edit/3 direct updates" do
    test "updates phone number", %{tenant: tenant, profile: profile} do
    ...
    test "updates email", %{tenant: tenant, profile: profile} do
      assert {:ok, msg} =
    ...
    test "updates business name", %{tenant: tenant, profile: profile} do
      assert {:ok, _} =
    ...
    test "updates service area", %{tenant: tenant, profile: profile} do
      assert {:ok, _} =
    ...
    test "updates owner name", %{tenant: tenant, profile: profile} do
      assert {:ok, _} =
  ...
  describe "apply_edit/3 service management" do
    test "removes a service by name (soft delete)", %{tenant: tenant, profile: profile} do
    ...
    test "returns error for nonexistent service", %{tenant: tenant, profile: profile} do
      assert {:error, msg} =
    ...
    test "adds a new service", %{tenant: tenant, profile: profile} do
  ...
  describe "apply_edit/3 regeneration" do
    test "regenerates tagline", %{tenant: tenant, profile: profile} do
      assert {:ok, msg} =
    ...
    test "regenerates service descriptions", %{tenant: tenant, profile: profile} do
      assert {:ok, msg} =
  ...
  describe "apply_edit/3 unknown" do
    test "returns help text for unknown edits", %{tenant: tenant, profile: profile} do
